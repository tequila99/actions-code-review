/**
 * `ReviewEngine` contract (verbatim field names/shapes for
 ...
 * original design, which reused the name `PullRequestInfo` for both).
 */
...
export interface ReviewContext {
  ...
  signal: AbortSignal
  /** FR-53: set only when `agent.filter_model` is configured — the same
...
export interface Finding {
  ...
  category: string
  message: string
  /** AgentEngine only (FR-51) — a verified-against-disk literal
...
export interface ReviewResult {
  ...
  notes: string[]
  truncated: boolean
  /** FR-53: findings dropped by the noise-filter pass. Always `0` for
